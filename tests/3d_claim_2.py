...
class Test3DBandlimitedReconstruction:
    ...
    @pytest.mark.parametrize("group_type,sub_group_type,subsampling_factor,expected_error,iterations, mode", [
        # Octahedral group subsampling
        ("octahedral", "cycle", 6, 1e-1, 10000, "gpu_optim"),  # O → C4 (24 → 4)
        
        # Full octahedral group subsampling
        ("full_octahedral", "cycle", 12, 1e-1, 10000, "gpu_optim"),  # O_h → C4 (48 → 4)
        ("full_octahedral", "dihedral", 6, 1e-1, 10000, "gpu_optim"),  # O_h → D4 (48 → 8)
        ("full_octahedral", "octahedral", 2, 1e-1, 10000, "gpu_optim"),  # O_h → O (48 → 24)
        
        # 2D groups on 3D data (for comparison)
        ("dihedral", "dihedral", 2, 1e-1, 10000, "gpu_optim"),  # D12 → D6 (24 → 12)
        ("cycle", "cycle", 2, 1e-1, 10000, "gpu_optim"),  # C12 → C6 (12 → 6)
    ])
